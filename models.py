...
def DNCNN_model (args):
    ...
    for i in range(args.num_layer):
        x = Conv2D(filters=64, kernel_size=(args.kernel_size,args.kernel_size), strides=(1,1), padding='same')(x)
        x = BatchNormalization(axis=-1, epsilon=1e-3)(x)
        x = Activation('relu')(x)   
    # last layer, Conv
    x = Conv2D(filters=1, kernel_size=(args.kernel_size,args.kernel_size), strides=(1,1), padding='same')(x)
